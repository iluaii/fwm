...
int main(int argc, char **argv) {
    if (argc < 2 || strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
        fprintf(stderr,
            ...
            "  theme              the UI palette in hex, and where it came from\n"
            ...
            "  urgent <d> [on|off]  light desktop d's number red until you go there\n"
            ...
            "\n"
            "urgent is the compositor's half of a notification: your daemon knows\n"
            "a message arrived, fwm knows which desktop it arrived ON. Desktops\n"
            "count from 0 here, as everywhere in the IPC — 0 is super+1:\n"
            "\n"
            "  fwmctl urgent 3              # the 4th number goes red\n"
            "  fwmctl urgent 3 off          # ...or put it out by hand\n"
            "\n"
            "It goes out on its own the moment that desktop is on a screen, so\n"
            "the usual dunst or mako rule is one line and nothing has to clear\n"
            "it. A desktop already in front of you is refused, and the reply\n"
            "says `\"urgent\":false` rather than failing. `state` lists the ones\n"
            "that are lit.\n"
            "\n"
            "subscribe takes all events by default, or a comma-separated subset:\n"
            "  window_open window_close window_focus window_title\n"
            "  desktop mode gravity config_reload setting ui palette urgent\n"
            "\n"
