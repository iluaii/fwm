...
- **A window opens where you asked for it** — not where you happen to be standing when it finally appears. The desktop is taken at the moment of the launch and kept against the process, so Steam started on desktop 2 and read about on desktop 1 while it loads still arrives on 2. Any descendant of the launch counts, since everything worth starting is a script that starts something else, and the claim expires after two minutes so a window opened by hand much later follows the camera like anything else. A `[[rule]] desktop` still outranks it.
...
- **Parallax wallpaper** — multi-layer background images (PNG/JPEG/WebP) that scroll as you move across desktops. `pan` mode turns one large image into a world you walk across, and `output` gives a monitor a wallpaper of its own.
...
### Built-in overlays
- **App launcher** (`Super+Space`) — fuzzy search over desktop entries with icons, no external `rofi` needed. It matches the way you actually type: the letters in order with gaps allowed (`ffx` finds Firefox), the keywords and the binary as well as the name (`browser` finds it too, so does `nvim`), the name your locale gives the entry, and case and accents folded — so a Russian query in capitals finds a Russian name. What it lists first is what you have been launching, aged by a half-life rather than counted forever, so the list follows habits that change and opens on today's applications rather than on whatever is alphabetically first. Launched windows drop into the world with physics.
...
fwmctl output DP-1 adaptive_sync=on               # VRR, where the panel has it
fwmctl output HDMI-A-1 allow_tearing=on           # and the other answer, where it has not
...
**A frame need not wait for the refresh.** A finished frame normally sits until the display asks for it — up to a whole refresh of nothing, which is the latency competitive players talk about. Two settings, per monitor, answer that in opposite directions. `adaptive_sync = true` is VRR (FreeSync, G-Sync): the display waits for the frame instead, so it arrives early and arrives whole, and where a panel has this there is nothing else to think about. `allow_tearing = true` is for the fixed-refresh panel that does not: a fullscreen client's frame goes on the glass the moment it is drawn, mid-scanout, old frame above the seam and new one below. Nothing is ever torn without being asked for three times over — the monitor allows it, the window is really fullscreen rather than fake, and the client itself requested it through `tearing-control-v1`, which is what a game does through Mesa when it runs without vsync. No other window on the desktop can tear, and a driver that cannot do an async flip simply refuses, one frame arriving whole instead.
